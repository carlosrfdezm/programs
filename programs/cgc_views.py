        ...
        if request.method == 'POST':
            try:
                ...
                if not fs.exists(new_brief_name):
                    ...
                    index =str(CGCBrief.objects.filter(year=request.POST['year'], month= request.POST['month']).__len__()+1)
...
@login_required()
def cgc_by_year_brief_zip_download(request, year):

    brief_list = CGCBrief.objects.filter( year=year)
    zipname = "Actas-de-CGC-"+str(year)
...
def cngc_brief_zip_download(request):
    ...
    brief_list = CNGCBrief.objects.all()
    zipname = "Actas-de-CNGC"
...
@login_required()
def cngc_by_year_brief_zip_download(request, year):

    brief_list = CGCBrief.objects.filter( year=year)
    zipname = "Actas-de-CNGC-"+str(year)
